...
- 460: prospective post-support assembly cancellation/provenance contract. Requires reporting `D=sum w_i F_i`, `S_abs=sum |w_i F_i|`, `kappa_cancel`, and the weighted cross-precision bound `B_80_120=sum |w_i||F_i^80-F_i^120|` independently for BASE/HALF, while retaining the frozen assembled scaled MP80↔MP120 threshold `<=2e-6`. Triangle-bound violation is implementation/provenance BLOCKED; large cancellation is diagnostic only. Non-promoting.
...
- 469: exact quartet cancellation decomposition. Refines the Iteration-460 sample-level condition number using Iteration-467 parity quartets: `|D|<=S_quartet<=S_sample`, hence `kappa_sample>=kappa_quartet>=1`; defines bounded `rho_parity=S_quartet/S_sample` and `rho_shell=|D|/S_quartet` with exact factorization `kappa_sample=1/(rho_parity*rho_shell)`. Diagnostic/provenance only; no physical promotion or threshold change.
- 470: raw PASS at Iteration-455 distinct rank 7 `(-5e-6,+1e-5)` from canonical run `33973849536`, job `101326982817`, artifact `9973031198`; occurrence-weighted certified coverage `12/32=37.5%`; non-promoting. Rank 8 `(+5e-6,-1e-5)` is the sole active next numerical gate.
- 471: exact MP80↔MP120 discrepancy quartet decomposition. For `DeltaF=F80-F120`, proves `|DeltaD|<=B_quartet_delta<=B_sample_delta`, refining the Iteration-460 weighted precision bound and localizing precision-discrepancy cancellation into parity-internal versus inter-quartet components. Diagnostic/provenance only; no threshold, estimator, dynamics, or physical promotion change.
- 472: exact local-to-assembled MP sufficiency contract. For local scaled errors `e_i<=eps_local`, proves `|DeltaD|<=B_sample_delta<=eps_local E` with `E=sum|w_i|max(1,|F80_i|,|F120_i|)`, hence a sufficient assembled condition `eps_local E/max(1,|D80|,|D120|)<=tau`. Gives an exact central4-sign counterexample showing local `1e-30` sample PASS can coexist with assembled scaled discrepancy `8/9`; therefore local PASS never substitutes for the frozen assembled `2e-6` gate. Provenance/conditioning only; non-promoting.
- 473: raw PASS at Iteration-455 distinct rank 8 `(+5e-6,-1e-5)` from canonical run `33980045356`, job `101343539742`, artifact `9974359345`; occurrence-weighted certified coverage `13/32=40.625%`; non-promoting. Rank 9 `(+5e-6,-5e-6)` multiplicity 2 is the sole active next numerical gate under unchanged frozen conventions.
- 474: exact mixed-derivative MP precision nullspace/double-centering audit. For `DeltaD=c^T DeltaF c` with frozen central4 `c`, proves `DeltaD=c^T P DeltaF P c`, `P=I-(1/4)11^T`; all row/column-separable discrepancy modes `a1^T+1b^T+gamma11^T` are exactly invisible, and `|DeltaD|<=(65/72)||P DeltaF P||_F`. Together with Iteration 472, local MP status alone is neither sufficient nor necessary for assembled MP status. Diagnostic/provenance only; no threshold, estimator, dynamics, support order, or physical promotion change.
