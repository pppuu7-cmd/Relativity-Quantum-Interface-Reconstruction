...
- Exact unresolved double-double physical set is **`[2,11]`**, both still `BLOCKED_CONVERGENCE` until a separately authorized physical reduction resolves them.
...
- **Iteration 410:** run `33847425175`, evaluator commit `08f3fc070f1073bd8558b0ba0facaffb300f880b`, workflow/head commit `f642954f1a0ded452b801decdb8667f6dc23c4f0`, is the active split resource-recovery oracle for indices 2 and 11.
- It imports Iteration-408 structural arithmetic verbatim and changes only execution granularity: one independent job per target.
- Frozen thresholds remain `2e-11` denominator-affine and `2e-6` Fourier-tail/phase-mean/held-out polynomial. It computes no physical `D_s` and cannot remove either physical blocker by itself.
- Iteration 410 is ACTIVE / NOT YET SCIENTIFIC AUTHORITY and must not be duplicated.
...
1. Raw-consume both Iteration-410 per-index artifacts fail-closed. Each can certify only structural executability for its own target.
2. For each structural PASS from Iteration 410, execute the same prospectively frozen Iteration-407 analytic/spectral physical reduction separately for that index, preserving the parent physical integrand, central4×central4 mass stencil, held-out original-integrand checks and unchanged `2e-5` physical convergence threshold.
...
Change through validated Iteration 409 / active Iteration 410: `0 pp`. Iteration 408's timeout is operational, not scientific; the split recovery does not itself close a readiness-rubric bucket. Complete `Tr U1^2`, linked Source/Ward/K2 closure and a robust comparator-subtracted residual remain open.
