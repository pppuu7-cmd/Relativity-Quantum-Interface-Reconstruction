...
- Iteration 335 is active and independent: it evaluates only the same unresolved `q^2=-1` triangle channel with Gauss-Legendre in `z=cos(theta)` times periodic azimuth quadrature plus phase-shift cross-check. Run `33753368856`; do not duplicate it.
...
- Iteration 337 calibrates that geometric factor against the repository's Iteration-296 scalar-bubble convention and freezes the ordinary simple-cut bridge `D_s I[F] = -8*pi int(dPhi2 F) = -sphere_mean(F)`. Validated run `33756194728`, job `100651082826`, artifact `9893533178`, digest `sha256:aaeca20e2906d240417b6c9d301639068c62076f11281694525a5263d1096161`, scientific JSON SHA-256 `7d6ba8fd46c01ccfb9af79b21932daa49787587122c47e51af85d8d7997bad64`.
...
**Iteration 335 remains active** on the sole unresolved triangle `q^2=-1` channel. It changes only the integration design; parent dynamics, timelike fixture, route-specific physical numerators, cut surface and frozen convergence threshold `2e-5` remain unchanged. Already-certified bubbles and the other two triangle channels are not recomputed.

If Iteration 335 passes the unchanged threshold, freeze the complete channel-resolved determinant absorptive vector and assemble the normalized determinant `e=0,c<=3` discontinuity using the Iteration-337/338 conversion `D_s Gamma_det = -i*sphere_mean` with family provenance. If Iteration 335 remains BLOCKED, derive a symbolic/analytic angular reduction; do not weaken thresholds.
