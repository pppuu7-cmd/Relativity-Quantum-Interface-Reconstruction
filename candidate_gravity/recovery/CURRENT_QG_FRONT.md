...
- Latest authoritative research iteration: Iteration 467.
...
## Iteration 467 exact parity-projector contract
For the frozen central4 coefficients, `c(-x)=-c(x)` exactly. Hence tensor weights obey `w(-u,v)=-w(u,v)`, `w(u,-v)=-w(u,v)`, and `w(-u,-v)=w(u,v)`. The mixed operator therefore annihilates arbitrary even-even, even-odd, and odd-even stencil sectors and retains only the odd-odd sector; this is an exact arbitrary-data statement, stronger than the degree<=4 synthetic moment probes of Iteration 462.

The canonical 16-term assembly is exactly equivalent to four signed quartets over positive node magnitudes, `F(+a,+b)-F(-a,+b)-F(+a,-b)+F(-a,-b)`, with dimensionless coefficients `4/9,-1/18,-1/18,1/144` for `(a,b)=(1,1),(1,2),(2,1),(2,2)` before the common `1/h^2` factor. After full support closure, canonical 16-term BASE/HALF assemblies at MP80 and MP120 must be cross-checked against this quartet representation. Mismatch beyond explicitly bounded arithmetic roundoff is implementation/provenance `BLOCKED`, not physics FAIL. This creates no `u<->v` identity, authorizes no support deduplication, and does not alter `ds=-d_base` or any frozen threshold.
