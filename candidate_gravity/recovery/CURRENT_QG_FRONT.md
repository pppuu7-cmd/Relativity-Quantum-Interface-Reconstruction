...
- Latest authoritative research iteration: **Iteration 471**.
...
- Active numerical gate: Iteration-455 distinct rank 8, `u=+5e-6, v=-1e-5`, source occurrence multiplicity 1; canonical run `33980045356` is in progress under unchanged frozen five-z/NPHI16/radial/direct-MP80/120 conventions and thresholds. Do not duplicate.
...
## Latest research closure — Iteration 471
Iteration 471 refines the Iteration-460 MP80↔MP120 weighted discrepancy provenance using the exact Iteration-467 parity quartets. For `DeltaF=F80-F120`, future assembled BASE/HALF evaluation must form `DeltaQ_ab`, `B_quartet_delta=sum |alpha_ab||DeltaQ_ab|`, and the existing sample-level `B_sample_delta`; exact arithmetic requires `|DeltaD| <= B_quartet_delta <= B_sample_delta`. Diagnostic ratios `rho_precision_parity=B_quartet_delta/B_sample_delta` and `rho_precision_shell=|DeltaD|/B_quartet_delta` localize cancellation of precision disagreement but are non-promoting. Any bound violation is implementation/provenance BLOCKED, never physics FAIL. No estimator or threshold changed.
