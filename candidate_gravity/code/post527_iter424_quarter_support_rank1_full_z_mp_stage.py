#!/usr/bin/env python3
"""Post-Iteration-527 prerequisite for the frozen Iteration-424 reevaluation.

Iteration 424 prospectively froze mass steps {5e-6,2.5e-6,1.25e-6} and true
80/120-digit fixed-node evaluation. BASE/HALF support is already raw-closed at
Iteration 527. The quarter step h=1.25e-6 has central4 nodes
{-2.5e-6,-1.25e-6,+1.25e-6,+2.5e-6}; four corner coordinates with both entries
in {+-2.5e-6} are already covered by HALF support, leaving 12 new coordinates.
This stage evaluates only the first untested quarter coordinate in deterministic
u-major/v-major source order: (-2.5e-6,-1.25e-6).

It is numerical provenance only and cannot promote physical index 2.
"""
...
result={'stage':'POST527_ITER424_QUARTER_SUPPORT_RANK1_FULL_Z_MP__UNNUMBERED_COLLISION_SAFE','classification':('PASS_ITER424_QUARTER_SUPPORT_RANK1_MP80_MP120__NON_PROMOTING' if passed else 'BLOCKED_ITER424_QUARTER_SUPPORT_RANK1_MP__NON_PROMOTING'),'scientific_gate_pass':passed,'promotes_physical_coordinate':False,'MODEL_READINESS':'24%','readiness_change_pp':0,'target':{'double_double_index':2,'class_id':3,'q_squared':-1.0},'frozen':{'iteration424_mass_step':H,'u':MASS_U,'v':MASS_V,'quarter_source_rank':1,'quarter_new_coordinate_count':12,'source_order':'quarter central4 4x4 u-major/v-major after exact HALF-overlap removal','z_samples':list(Z_SAMPLES),'phi_nodes':NPHI,'radial_hs':list(RADIAL_HS),'precision_digits':[80,120]},'thresholds':{'scaled_mp80_vs_mp120_max':'1e-30','radial_richardson_scaled_max':RADIAL_LIMIT,'required_sample_count':expected,'all_finite':True},'observed':{'scaled_mp80_vs_mp120_max':mp.nstr(mx,30),'max_radial_richardson_scaled_error':mp.nstr(maxrad,30),'sample_count':len(rows),'all_finite':finite,'runtime_seconds':time.perf_counter()-start},'rows':rows,'next_gate_if_pass':'raw-consume non-promoting, then advance to quarter source rank2 only; after all 12 new coordinates are raw-closed, execute the frozen Iteration424 three-step physical reevaluation with all five clauses','next_gate_if_blocked':'localize first failing z/phi/radial sample at quarter rank1 without changing conventions','guardrails':['ITERATION527_RAW_ASSEMBLY_PASS_REQUIRED','ITERATION424_FROZEN_MASS_STEPS','ONE_NEW_QUARTER_COORDINATE_ONLY','EXACT_HALF_OVERLAPS_REUSED_ONLY_BY_COORDINATE_IDENTITY','DIRECT_PARENT_MP80_MP120','NO_PHYSICAL_DS_PROMOTION','NO_THRESHOLD_WEAKENING','NO_ZERO_FILL','NO_ANSATZ003','NO_FISHER_RESOURCES']}
