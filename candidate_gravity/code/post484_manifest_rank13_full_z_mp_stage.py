...
auth=json.loads((R/'post484_rank12_raw_consumption.json').read_text())
if auth.get('scientific_gate_pass') is not True or auth.get('classification')!='PASS_RAW_CONSUMED_MANIFEST_RANK12_FULL_Z_MP80_MP120__NON_PROMOTING': raise SystemExit('rank12 prerequisite not passed')
t=auth.get('target',{})
if t.get('manifest_rank')!=12 or abs(float(t.get('u'))-1e-5)>1e-18 or abs(float(t.get('v'))+1e-5)>1e-18: raise SystemExit(('rank12_prerequisite_drift',t))
...
C={'__name__':'post484_rank13_parent','__file__':str(p)}
...
N={'__name__':'post484_mass_binding','__file__':str(p379)}
...
MASS_U=+2.0*BASE_H; MASS_V=-1.0*BASE_H; Z_SAMPLES=(-0.86,-0.43,0.0,0.43,0.86)
...
result={'stage':'POST484_MANIFEST_RANK13_FULL_Z_MP_STAGE__UNNUMBERED_COLLISION_SAFE','classification':('PASS_NEXT_MASS_NODE_FULL_Z_MP80_MP120__NON_PROMOTING' if passed else 'BLOCKED_NEXT_MASS_NODE_FULL_Z_MP__NON_PROMOTING'),'scientific_gate_pass':passed,'promotes_physical_coordinate':False,'MODEL_READINESS':'24%','readiness_change_pp':0,'target':{'double_double_index':2,'class_id':3,'q_squared':-1.0},'frozen':{'u':MASS_U,'v':MASS_V,'source_order_rule':'Iteration455 distinct rank 13','source_occurrence_multiplicity':1,'base_h':BASE_H,'z_samples':list(Z_SAMPLES),'phi_nodes':NPHI,'radial_hs':list(RADIAL_HS),'precision_digits':[80,120]},'thresholds':{'scaled_mp80_vs_mp120_max':'1e-30','radial_richardson_scaled_max':RADIAL_LIMIT,'required_sample_count':expected,'all_finite':True},'observed':{'scaled_mp80_vs_mp120_max':mp.nstr(mx,30),'max_radial_richardson_scaled_error':mp.nstr(maxrad,30),'sample_count':len(rows),'all_finite':finite,'runtime_seconds':time.perf_counter()-start},'rows':rows,'next_gate_if_pass':'raw-consume non-promoting, then advance only to next UNTESTED Iteration455 manifest coordinate','next_gate_if_blocked':'localize first failing z/phi/radial sample at u=+1e-5,v=-5e-6','guardrails':['RANK12_RAW_PASS_REQUIRED','ONE_MASS_COORDINATE_ONLY','ITERATION455_MANIFEST_ORDER','SKIP_ALREADY_CERTIFIED_COORDINATES','NO_BLIND_REMAINING_GRID_SWEEP','NO_PHYSICAL_DS_PROMOTION','NO_THRESHOLD_WEAKENING','NO_ZERO_FILL','NO_ANSATZ003','NO_FISHER_RESOURCES']}
