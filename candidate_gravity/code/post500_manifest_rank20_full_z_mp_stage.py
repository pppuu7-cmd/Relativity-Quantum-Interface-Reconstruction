...
auth=json.loads((R/'post500_rank19_raw_consumption.json').read_text())
if auth.get('scientific_gate_pass') is not True or auth.get('classification')!='PASS_RAW_CONSUMED_MANIFEST_RANK19_FULL_Z_MP80_MP120__NON_PROMOTING': raise SystemExit('rank19 prerequisite not passed')
t=auth.get('target',{})
if t.get('manifest_rank')!=19 or abs(float(t.get('u'))+2.5e-6)>1e-18 or abs(float(t.get('v'))+2.5e-6)>1e-18: raise SystemExit(('rank19_prerequisite_drift',t))
...
C={'__name__':'post500_rank20_parent','__file__':str(p)}
...
N={'__name__':'post500_mass_binding','__file__':str(p379)}
