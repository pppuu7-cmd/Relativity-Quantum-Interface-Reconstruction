#!/usr/bin/env python3
"""Post-Iteration-556 frozen Iteration-424 quarter-support rank9 stage."""
from __future__ import annotations
import contextlib, io, json, math, time
from pathlib import Path
import mpmath as mp
ROOT=Path(__file__).resolve().parent; R=ROOT.parent/'results'
auth=json.loads((R/'iteration556_iter424_quarter_rank8_raw_consumption.json').read_text())
if (auth.get('iteration')!=556 or auth.get('classification')!='PASS_RAW_CONSUMED_ITER424_QUARTER_SUPPORT_RANK8_MP80_MP120__NON_PROMOTING' or auth.get('conclusions',{}).get('rank8_raw_valid_pass') is not True or auth.get('conclusions',{}).get('rank9_authorized_by_iteration532_order') is not True or auth.get('next_rank',{}).get('quarter_source_rank')!=9 or abs(float(auth.get('next_rank',{}).get('u'))-1.25e-6)>1e-18 or abs(float(auth.get('next_rank',{}).get('v'))-1.25e-6)>1e-18): raise SystemExit(('iteration556_rank8_raw_consumption_prerequisite_not_passed',auth))
...
C={'__name__':'post556_iter424_quarter_rank9_parent','__file__':str(p)}
...
MASS_U,MASS_V=untested[8]
if abs(MASS_U-1.25e-6)>1e-18 or abs(MASS_V-1.25e-6)>1e-18: raise SystemExit(('rank9_coordinate_drift',MASS_U,MASS_V))
...
result={'stage':'POST556_ITER424_QUARTER_SUPPORT_RANK9_FULL_Z_MP__UNNUMBERED_COLLISION_SAFE','classification':('PASS_ITER424_QUARTER_SUPPORT_RANK9_MP80_MP120__NON_PROMOTING' if passed else 'BLOCKED_ITER424_QUARTER_SUPPORT_RANK9_MP__NON_PROMOTING'),'scientific_gate_pass':passed,'promotes_physical_coordinate':False,'MODEL_READINESS':'24%','readiness_change_pp':0,'target':{'double_double_index':2,'class_id':3,'q_squared':-1.0},'frozen':{'iteration424_mass_step':H,'u':MASS_U,'v':MASS_V,'quarter_source_rank':9,'quarter_new_coordinate_count':12,'source_order':'Iteration532 frozen quarter central4 u-major/v-major after exact HALF-overlap removal','z_samples':list(Z_SAMPLES),'phi_nodes':NPHI,'radial_hs':list(RADIAL_HS),'precision_digits':[80,120]},'thresholds':{'scaled_mp80_vs_mp120_max':'1e-30','radial_richardson_scaled_max':RADIAL_LIMIT,'required_sample_count':expected,'all_finite':True},'observed':{'scaled_mp80_vs_mp120_max':mp.nstr(mx,30),'max_radial_richardson_scaled_error':mp.nstr(maxrad,30),'sample_count':len(rows),'all_finite':finite,'runtime_seconds':time.perf_counter()-start},'rows':rows,'next_gate_if_pass':'raw-consume non-promoting, then advance only to Iteration532 quarter source rank10 (+1.25e-6,+2.5e-6)','next_gate_if_blocked':'localize first failing z/phi/radial sample at quarter rank9 without changing conventions','guardrails':['ITERATION556_RANK8_RAW_CONSUMPTION_PASS_REQUIRED','ITERATION532_SUCCESSOR_ORDER','ITERATION424_FROZEN_MASS_STEPS','ONE_NEW_QUARTER_COORDINATE_ONLY','DIRECT_PARENT_MP80_MP120','NO_PHYSICAL_DS_PROMOTION','NO_THRESHOLD_WEAKENING','NO_ZERO_FILL','NO_UV_SYMMETRY_SUBSTITUTION','NO_ANSATZ003','NO_FISHER_RESOURCES']}
