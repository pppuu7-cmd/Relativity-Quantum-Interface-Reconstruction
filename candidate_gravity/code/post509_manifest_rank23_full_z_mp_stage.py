...
auth=json.loads((R/'post508_rank22_raw_consumption.json').read_text())
if auth.get('scientific_gate_pass') is not True or auth.get('classification')!='PASS_RAW_CONSUMED_MANIFEST_RANK22_FULL_Z_MP80_MP120__NON_PROMOTING': raise SystemExit('rank22 prerequisite not passed')
t=auth.get('target',{})
if t.get('manifest_rank')!=22 or abs(float(t.get('u'))-2.5e-6)>1e-18 or abs(float(t.get('v'))+5e-6)>1e-18: raise SystemExit(('rank22_prerequisite_drift',t))
nxt=auth.get('next_manifest_coordinate',{})
if nxt.get('manifest_rank')!=23 or abs(float(nxt.get('u'))-2.5e-6)>1e-18 or abs(float(nxt.get('v'))+2.5e-6)>1e-18 or nxt.get('source_occurrence_multiplicity')!=1 or nxt.get('source_level')!='HALF' or nxt.get('source_local_index')!=9: raise SystemExit(('rank23_manifest_successor_drift',nxt))
...
C={'__name__':'post509_rank23_parent','__file__':str(p)}
...
N={'__name__':'post509_mass_binding','__file__':str(p379)}
...
result={'stage':'POST509_MANIFEST_RANK23_FULL_Z_MP_STAGE__UNNUMBERED_COLLISION_SAFE','classification':('PASS_NEXT_MASS_NODE_FULL_Z_MP80_MP120__NON_PROMOTING' if passed else 'BLOCKED_NEXT_MASS_NODE_FULL_Z_MP__NON_PROMOTING'),'scientific_gate_pass':passed,'promotes_physical_coordinate':False,'MODEL_READINESS':'24%','readiness_change_pp':0,'target':{'double_double_index':2,'class_id':3,'q_squared':-1.0},'frozen':{'u':MASS_U,'v':MASS_V,'source_order_rule':'Iteration455 distinct rank 23','source_occurrence_multiplicity':1,'source_level':'HALF','source_local_index':9,'base_h':BASE_H,'z_samples':list(Z_SAMPLES),'phi_nodes':NPHI,'radial_hs':list(RADIAL_HS),'precision_digits':[80,120]},'thresholds':{'scaled_mp80_vs_mp120_max':'1e-30','radial_richardson_scaled_max':RADIAL_LIMIT,'required_sample_count':expected,'all_finite':True},'observed':{'scaled_mp80_vs_mp120_max':mp.nstr(mx,30),'max_radial_richardson_scaled_error':mp.nstr(maxrad,30),'sample_count':len(rows),'all_finite':finite,'runtime_seconds':time.perf_counter()-start},'rows':rows,'next_gate_if_pass':'raw-consume non-promoting, then advance only to preregistered Iteration455 manifest rank24','next_gate_if_blocked':'localize first failing z/phi/radial sample at u=+2.5e-6,v=-2.5e-6','guardrails':['RANK22_RAW_PASS_REQUIRED','ONE_MASS_COORDINATE_ONLY','ITERATION455_MANIFEST_ORDER','SKIP_ALREADY_CERTIFIED_COORDINATES','NO_BLIND_REMAINING_GRID_SWEEP','NO_PHYSICAL_DS_PROMOTION','NO_THRESHOLD_WEAKENING','NO_ZERO_FILL','NO_ANSATZ003','NO_FISHER_RESOURCES']}
