...
auth=json.loads((R/'iteration473_mass_node_raw_consumed.json').read_text())
if auth.get('authoritative_iteration')!=473 or auth.get('scientific_gate_pass') is not True: raise SystemExit('iteration473 prerequisite not passed')
coord=auth.get('next_gate',{})
if coord.get('distinct_rank')!=9 or abs(float(coord.get('u'))-5e-6)>1e-18 or abs(float(coord.get('v'))+5e-6)>1e-18: raise SystemExit(('rank9_manifest_prerequisite_drift',coord))
...
C={'__name__':'post473_rank9_parent','__file__':str(p)}
...
N={'__name__':'post473_mass_binding','__file__':str(p379)}
