...
auth=json.loads((R/'post479_rank11_raw_consumption.json').read_text())
if auth.get('scientific_gate_pass') is not True or auth.get('classification')!='PASS_RAW_CONSUMED_MANIFEST_RANK11_FULL_Z_MP80_MP120__NON_PROMOTING': raise SystemExit('rank11 prerequisite not passed')
t=auth.get('target',{})
if t.get('manifest_rank')!=11 or abs(float(t.get('u'))-5e-6)>1e-18 or abs(float(t.get('v'))-1e-5)>1e-18: raise SystemExit(('rank11_prerequisite_drift',t))
...
C={'__name__':'post480_rank12_parent','__file__':str(p)}
...
N={'__name__':'post480_mass_binding','__file__':str(p379)}
