...
auth=json.loads((R/'post502_rank20_raw_consumption.json').read_text())
if auth.get('scientific_gate_pass') is not True or auth.get('classification')!='PASS_RAW_CONSUMED_MANIFEST_RANK20_FULL_Z_MP80_MP120__NON_PROMOTING': raise SystemExit('rank20 prerequisite not passed')
t=auth.get('target',{})
if t.get('manifest_rank')!=20 or abs(float(t.get('u'))+2.5e-6)>1e-18 or abs(float(t.get('v'))-2.5e-6)>1e-18: raise SystemExit(('rank20_prerequisite_drift',t))
nxt=auth.get('next_manifest_coordinate',{})
if nxt.get('manifest_rank')!=21 or abs(float(nxt.get('u'))+2.5e-6)>1e-18 or abs(float(nxt.get('v'))-5e-6)>1e-18 or nxt.get('source_occurrence_multiplicity')!=1: raise SystemExit(('rank21_manifest_successor_drift',nxt))
...
C={'__name__':'post503_rank21_parent','__file__':str(p)}
...
N={'__name__':'post503_mass_binding','__file__':str(p379)}
