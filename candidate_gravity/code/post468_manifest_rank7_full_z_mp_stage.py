...
auth=json.loads((R/'iteration468_mass_node_raw_consumed.json').read_text())
if auth.get('authoritative_iteration')!=468 or auth.get('scientific_gate_pass') is not True: raise SystemExit('iteration468 prerequisite not passed')
coord=auth.get('next_gate',{})
if coord.get('distinct_rank')!=7 or abs(float(coord.get('u'))+5e-6)>1e-18 or abs(float(coord.get('v'))-1e-5)>1e-18: raise SystemExit(('rank7_manifest_prerequisite_drift',coord))
...
C={'__name__':'post468_rank7_parent','__file__':str(p)}
...
N={'__name__':'post468_mass_binding','__file__':str(p379)}
