...
auth=json.loads((R/'post486_rank13_raw_consumption.json').read_text())
if auth.get('scientific_gate_pass') is not True or auth.get('classification')!='PASS_RAW_CONSUMED_MANIFEST_RANK13_FULL_Z_MP80_MP120__NON_PROMOTING': raise SystemExit('rank13 prerequisite not passed')
t=auth.get('target',{})
if t.get('manifest_rank')!=13 or abs(float(t.get('u'))-1e-5)>1e-18 or abs(float(t.get('v'))+5e-6)>1e-18: raise SystemExit(('rank13_prerequisite_drift',t))
...
C={'__name__':'post486_rank14_parent','__file__':str(p)}
...
N={'__name__':'post486_mass_binding','__file__':str(p379)}
