...
auth=json.loads((R/'post489_rank14_raw_consumption.json').read_text())
if auth.get('scientific_gate_pass') is not True or auth.get('classification')!='PASS_RAW_CONSUMED_MANIFEST_RANK14_FULL_Z_MP80_MP120__NON_PROMOTING': raise SystemExit('rank14 prerequisite not passed')
t=auth.get('target',{})
if t.get('manifest_rank')!=14 or abs(float(t.get('u'))-1e-5)>1e-18 or abs(float(t.get('v'))-5e-6)>1e-18: raise SystemExit(('rank14_prerequisite_drift',t))
...
C={'__name__':'post489_rank15_parent','__file__':str(p)}
...
N={'__name__':'post489_mass_binding','__file__':str(p379)}
...
MASS_U=+2.0*BASE_H; MASS_V=+2.0*BASE_H; Z_SAMPLES=(-0.86,-0.43,0.0,0.43,0.86)
