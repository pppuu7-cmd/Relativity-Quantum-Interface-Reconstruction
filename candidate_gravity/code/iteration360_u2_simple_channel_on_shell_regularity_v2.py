#!/usr/bin/env python3
"""RQIR Candidate Gravity Iteration 360.

New-version replacement for operationally failed Iteration 358.  Physics scope,
fixtures, channel census and thresholds are unchanged.  The only correction is
that Minkowski bilinear products are evaluated by an explicit bilinear helper;
the inherited mdot from Iteration 355 is norm-only.
...
with contextlib.redirect_stdout(io.StringIO()):
    P355=runpy.run_path(str(ROOT/'iteration355_u2_heldout_physical_numerator_transport.py'),run_name='iteration360_parent355')
    P356=runpy.run_path(str(ROOT/'iteration356_u2_family_origin_topology_classification.py'),run_name='iteration360_parent356')
    P357=runpy.run_path(str(ROOT/'iteration357_u2_simple_vs_repeated_pole_cut_contract.py'),run_name='iteration360_parent357')
raw=P355['raw']; enumerate_subterms=P355['enumerate_subterms']; mdot=P355['mdot']
TIMELIKE_TOL=2e-12; UNCUT_SEPARATION_TOL=1e-10; NUMERATOR_FINITE_LIMIT=1e100
...
def transverse_basis(q):
    seeds=[np.array([0.,1.,0.,0.]),np.array([0.,0.,1.,0.]),np.array([0.,0.,0.,1.]),np.array([1.,0.,0.,0.]),np.array([1.,1.,0.,0.]),np.array([1.,0.,1.,0.])]
    basis=[]
    for s in seeds:
        v=mproj_orth(s,q)
    ...
    gram=np.array([[float(np.real(mbilin(a,b))) for b in basis] for a in basis])
    if np.max(np.abs(gram-np.eye(3)))>2e-10: raise RuntimeError(('bad_transverse_gram',gram.tolist()))
    if max(abs(float(np.real(mbilin(e,q)))) for e in basis)>2e-10: raise RuntimeError('basis_not_q_orthogonal')
    return basis
...
for fam in simple_records:
    ...
    for ch in fam['timelike_pair_channels']:
        ...
        channels.append({'route':route,'subterm':subterm,'cut_pair':[i,j],'q2':q2,'status':status,
                         'numerator_structure':'STRIPPED_LOCAL_POLYNOMIAL_MATRIX_PRODUCT','numerator_sample_count':len(vals),
                         'numerator_sample_abs_min':min_num,'numerator_sample_abs_max':max_num,'numerator_finite_on_all_samples':finite,
                         'cut_shell_max_abs_error':local_shell_error,'uncut_denominators':uncut,
...
result={'iteration':ITERATION,'model_readiness_percent':24,'scientific_gate_pass':passed,'candidate_residual':False,
 'classification':('PASS_U2_ORDINARY_SIMPLE_CHANNEL_ON_SHELL_REGULARITY_AND_UNCUT_DENOMINATOR_CLASSIFICATION_V2' if passed else 'FAIL_U2_ORDINARY_SIMPLE_CHANNEL_ON_SHELL_DIAGNOSTIC_GATE_V2'),
 'census':{'ordinary_simple_families':12,'typed_channels':len(channels),'REGULAR':regular,'ZERO':zero,'BLOCKED':blocked,'max_cut_shell_abs_error':max_shell_error,'minimum_certified_uncut_abs_r2':min_certified_uncut},
 ...
 'channels':channels,'scope':'ORDINARY_SIMPLE_U2_ON_SHELL_DIAGNOSTICS_ONLY__ITERATION358_OPERATIONAL_FIX_ONLY__NO_DISCONTINUITY_INTEGRATION',
 'guardrails':['ITERATION358_OPERATIONAL_MD0T_ARITY_FAIL_PRESERVED','PHYSICS_THRESHOLDS_UNCHANGED_FROM_ITERATION358','ITERATION357_SIMPLE_ONLY','REPEATED_POLE_FAMILIES_NOT_EVALUATED','ANALYTIC_UNCUT_DENOMINATOR_RANGE_NOT_SAMPLED_ZERO_TEST','NO_ZERO_FROM_NUMERICAL_SAMPLING','UNSUPPORTED_IS_BLOCKED_NOT_ZERO','NO_SOURCE_BORN_SUBTRACTION','NO_ANSATZ003','NO_FISHER_RESOURCES','NO_BLIND_FULL_C5'],
 'next_gate':'for REGULAR ordinary-simple channels only, perform normalized channel-resolved Tr U2 discontinuity using frozen Iterations 336-338 normalization; keep BLOCKED simple channels separate; repeated-pole branch follows Iteration 359 authority if/when that independent gate passes'}
