...
auth=json.loads((R/'iteration463_mass_node_raw_consumed.json').read_text())
if auth.get('authoritative_iteration')!=463 or auth.get('scientific_gate_pass') is not True: raise SystemExit('iteration463 prerequisite not passed')
...
C={'__name__':'post463_rank5_parent','__file__':str(p)}
...
N={'__name__':'post463_mass_binding','__file__':str(p379)}
...
result={'stage':'POST463_MANIFEST_RANK5_FULL_Z_MP_STAGE__UNNUMBERED_COLLISION_SAFE','classification':('PASS_NEXT_MASS_NODE_FULL_Z_MP80_MP120__NON_PROMOTING' if passed else 'BLOCKED_NEXT_MASS_NODE_FULL_Z_MP__NON_PROMOTING'),'scientific_gate_pass':passed,'promotes_physical_coordinate':False,'MODEL_READINESS':'24%','readiness_change_pp':0,'target':{'double_double_index':2,'class_id':3,'q_squared':-1.0},'frozen':{'u':MASS_U,'v':MASS_V,'source_order_rule':'Iteration455 distinct rank 5','source_occurrence_multiplicity':2,'base_h':BASE_H,'z_samples':list(Z_SAMPLES),'phi_nodes':NPHI,'radial_hs':list(RADIAL_HS),'precision_digits':[80,120]},'thresholds':{'scaled_mp80_vs_mp120_max':'1e-30','radial_richardson_scaled_max':RADIAL_LIMIT,'required_sample_count':expected,'all_finite':True},'observed':{'scaled_mp80_vs_mp120_max':mp.nstr(mx,30),'max_radial_richardson_scaled_error':mp.nstr(maxrad,30),'sample_count':len(rows),'all_finite':finite,'runtime_seconds':time.perf_counter()-start},'rows':rows,'next_gate_if_pass':'raw-consume non-promoting, then advance to manifest distinct rank 6 only','next_gate_if_blocked':'localize first failing z/phi/radial sample at u=-5e-6,v=-5e-6','guardrails':['ITERATION463_RAW_PASS_REQUIRED','ONE_MASS_COORDINATE_ONLY','ITERATION455_MANIFEST_ORDER','EXACT_BASE_HALF_OVERLAP_CERTIFICATE_SHARED_BUT_WEIGHTS_DISTINCT','NO_BLIND_REMAINING_GRID_SWEEP','NO_PHYSICAL_DS_PROMOTION','NO_THRESHOLD_WEAKENING','NO_ZERO_FILL','NO_ANSATZ003','NO_FISHER_RESOURCES']}
