\section{Resource conversion after nuisance profiling}
...
\end{equation}
Equation~\eqref{eq:resource-variational} is the resource handoff from RQIR II. The nuisance fit is performed for each candidate resource allocation. This differs from optimizing a raw target-target Fisher element and profiling only afterward.
...
For the RQIR-II two-band target $\bm g=(g_2,g_4)^T$ and relative-tilt nuisance $\bm t=(g_2,-g_4)^T$, independent exposures $e_2,e_4$ give
\begin{equation}
