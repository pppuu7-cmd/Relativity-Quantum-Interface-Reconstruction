...
The boundary with the remaining RQIR papers is deliberate. Paper IV will use the completed calibration, nuisance, and resource gates as a common comparator funnel for established classical, semiclassical, stochastic, hybrid, postquantum, and quantum gravity/interface models. Its purpose is to determine which known models pass, fail, or remain operationally degenerate under the same declared gates rather than to introduce a preferred new model. Only after that benchmark layer is complete does Paper V construct and present the KG model against the requirements and failure modes exposed by Papers I--IV. Resource closure in the present paper is therefore a necessary input to model comparison, but it is neither comparator exclusion nor the KG construction itself.
...
The five-paper inference program is therefore explicit:
\begin{equation}
\begin{split}
&\text{Paper I: calibration survival}
\rightarrow
\text{Paper II: statistical identifiability}
\\
&\rightarrow
\text{Paper III: resource closure}
\rightarrow
\text{Paper IV: known-model comparator funnel}
\\
&\rightarrow
\text{Paper V: constructive KG model}.
\end{split}
\end{equation}
RQIR III supplies the resource-closure layer. Apparatus-specific reach requires the remaining binding of scores and costs to measured spectral densities, coherence, geometry, control overhead, and feasible laboratory budgets. Known-model discrimination is deferred to Paper IV, and construction of the KG model is deferred to Paper V so that the new model is judged against a funnel whose operational, statistical, and resource requirements were fixed in advance.
...
\bibitem{Schilling2020}
M. Schilling, E. Wodey, L. Timmen, D. Tell, K. H. Zipfel,
D. Schlippert, C. Schubert, E. M. Rasel, and J. M\"uller,
``Gravity field modelling for the Hannover 10 m atom interferometer,''
J. Geod. \textbf{94}, 122 (2020),
