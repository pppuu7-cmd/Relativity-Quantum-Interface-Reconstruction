\section{Methods: source-grounded Raman atom-gravimeter model}
\label{sec:apparatus}
...
\end{align}
At four shots per second, the measured $11$ mrad/shot therefore implies $1.39247\times10^{-8}g$ at $1$ s, reproducing the published $1.4\times10^{-8}g$ scale to rounding accuracy. This reproduction is a dimensional consistency check, not a fit. Throughout the apparatus forecasts, ``ng'' and ``$\mu g$'' denote acceleration amplitudes $10^{-9}g$ and $10^{-6}g$, respectively, with $g=9.80665\,{\rm m\,s^{-2}}$; they are not mass units.

\subsection{Declared science/reference geometry}

To make the estimability claim reproducible from the article rather than only from code, we state the modulation geometry explicitly. For shot index $i$,
\begin{equation}
t_i=iT_c,\qquad x_i\in[-1,1],
\end{equation}
with $x_i$ linearly spanning the campaign. The declared science and reference templates are
\begin{align}
s_i&=R_a(f_s)\sin(2\pi f_s t_i), & f_s&=0.5\ {\rm Hz},\\
q_i&=R_a(f_r)\sin(2\pi f_r t_i+0.37), & f_r&=1.0\ {\rm Hz}.
\label{eq:templates}
\end{align}
The nominal acceleration-equivalent reference amplitude is $\alpha=1\,\mu g$. Writing
\begin{equation}
K_g=k_{\rm eff}T^2g,
\end{equation}
the local phase model used by the detector-facing calculation is
\begin{equation}
\Phi_i=\frac{\pi}{2}+K_g(1+\gamma)
\left(\theta_g s_i+\alpha_g q_i\right)
+\phi_0+\phi_1x_i+\phi_2x_i^2,
\label{eq:phase-model}
\end{equation}
where $\theta_g$ and $\alpha_g$ are the science and reference amplitudes expressed in units of $g$, $\gamma$ is a shared multiplicative acceleration/phase-scale nuisance, and $\phi_0,\phi_1,\phi_2$ are offset and drift coefficients. In the numerical implementation $\theta_g=10^{-9}\theta_{\rm ng}$ and $\alpha_g=10^{-6}\alpha_{\mu g}$. For the static-science negative control, $s_i$ is replaced by unity. For the no-reference control, $q_i$ is set to zero; for the unconstrained-reference control, $\alpha$ is promoted to a free nuisance without an external precision term.

This geometry deliberately separates the $0.5$-Hz science modulation from the $1$-Hz reference modulation while retaining one shared multiplicative scale. The reference phase offset $0.37$ rad is fixed prospectively and is not fitted to improve the Fisher result. The choice is a design architecture, not a claim that these exact modulation frequencies were used in the historical 2008 campaign.

\subsection{From a published vibration spectrum to sampled covariance}

The same apparatus paper reports the measured passive-platform acceleration spectrum and the propagation of vibration noise through the interferometer \cite{LeGouet2008}. In the calculation, a coarse log--log digitization of the passive-platform curve is fixed explicitly as an approximate source-traceable digitization, not as raw campaign data. No amplitude normalization is fitted to the integrated sensitivity.
...
\begin{equation}
\sigma_{a,\rm vib}(1\,{\rm s})=6.34856\times10^{-8}g,
\end{equation}
...
\begin{equation}
\sigma_{a,\rm corr}(1\,{\rm s})=2.17592\times10^{-8}g,
\end{equation}
