...
Date: 2026-09-08
Status: submission control document / v0.4
...
| C6 | Calibration geometry can rotate the hidden direction according to the stated perturbative steering relation in its declared setting. | Analytic + optional constructive | Local constant-rank derivation; normalized null-vector gauge; pseudoinverse and conditioning assumptions. | GREEN (analytic) | The derivation is now in v0.4; numerical steering illustration remains optional. |
...
5. Promote exact toy residuals/values into the manuscript and figures only after steps 1–2.
