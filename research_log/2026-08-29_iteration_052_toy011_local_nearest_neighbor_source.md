# RQIR Research Log — Iteration 052 / Toy011
...
Use the same five radii and exact spectrum, but generate an exact nearest-neighbor Jacobi-chain Hamiltonian by Lanczos tridiagonalization of `diag(E)` from a positive cyclic vector `q0`.

A deterministic 12000-trial scan (`seed=20260902`) jointly varies:
...
- D1 two-band source proxy `~12.2%` of current Toy009/Iter011;
- D2 proxy `~15.6%`.
...
However, the current local-source Pareto points pay a substantial detector-information penalty.
